...
public class RegisterScript : MonoBehaviour {
	...
	bool CheckDatabase(string user, string password)
	{
		...
		StreamWriter DATABASE = new StreamWriter(path, true);
		DATABASE.WriteLine (user + " " + password);
		DATABASE.Close ();

		// create the default user data and exp 
		path = Application.persistentDataPath + "/personal_user";
		StreamWriter USER_EXP_DATABASE = new StreamWriter(path, true);
		USER_EXP_DATABASE.WriteLine (user + " 00000 0 0 0 0 0 0 0 0");
		USER_EXP_DATABASE.Close ();
